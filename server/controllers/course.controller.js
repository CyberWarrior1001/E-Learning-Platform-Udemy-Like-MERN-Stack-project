...
export const togglePublishCourse = async (req, res) => {
  try {
    const { courseId } = req.params;
    const { publish } = req.query; // true, fasle
...
export const removeCourse = async (req, res) => {
  ...
  try {
    ...
    const getPublicId = (url) => {
      ...
      const fileName = parts.pop(); // ex: "image.png"
      return fileName.split(".")[0]; // ex: "image"
    };

    // 2. Delete course thumbnail from Cloudinary
    const thumbnailPublicId = getPublicId(course.courseThumbnail);
    if (thumbnailPublicId) {
      await cloudinary.uploader.destroy(thumbnailPublicId);
    }

    // 3. Delete lecture videos + thumbnails from Cloudinary
    for (const lecture of course.lectures) {
      if (lecture.videoUrl) {
        const videoPublicId = getPublicId(lecture.videoUrl);
      ...
      if (lecture.thumbnail) {
        const thumbPublicId = getPublicId(lecture.thumbnail);
    ...
    return res.status(200).json({
      message: "Course and its lectures have been removed successfully!",
      success: true,
    });
  } catch (error) {
    console.log(error);
