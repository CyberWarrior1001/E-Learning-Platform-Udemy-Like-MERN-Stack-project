...
export const updateLectureProgress = async (req, res) => {
    try {
        ...
        let courseProgress = await CourseProgress.findOne({courseId, userId})
         if(!courseProgress){
             // If no progress exist creat a new record
        ...
        // if all lecture is compleated 

        const lectureProgressLength = courseProgress.lectureProgress.filter((lectrueProg) => lectrueProg.viewed).length
        ...
        return res.status(200).json({
            message: "Lectrue progress updated successfully."
        })
